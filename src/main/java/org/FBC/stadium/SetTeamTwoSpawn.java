...
public class SetTeamTwoSpawn {
    private Main plugin;

    public SetTeamTwoSpawn(String StadName, String teamTwoSpawn, Main p) {
        this.plugin = p;
        File StadFile = this.getStadFile(StadName);
        YamlConfiguration StadConfig = YamlConfiguration.loadConfiguration((File)StadFile);
        StadConfig.set("StadData.Spawns.teamTwoSpawn", (Object)teamTwoSpawn);
        try {
