...
public class SetTeamOneSpawn {
    private Main plugin;

    public SetTeamOneSpawn(String StadName, String teamOneSpawn, Main p) {
        this.plugin = p;
        File StadFile = this.getStadFile(StadName);
        YamlConfiguration StadConfig = YamlConfiguration.loadConfiguration((File)StadFile);
        StadConfig.set("StadData.Spawns.teamOneSpawn", (Object)teamOneSpawn);
        try {
