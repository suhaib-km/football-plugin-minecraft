...
public class StadiumCreate {
    ...
    public StadiumCreate(String StadName, Boolean ranked, Main p) {
        this.plugin = p;
        File StadFile = this.getStadFile(StadName);
        if (!StadFile.exists()) {
            try {
                StadFile.createNewFile();
            }
            catch (IOException e) {
                Bukkit.getLogger().log(Level.WARNING, "Could not create the file for the stadium " + StadName);
                return;
            }
        }
        YamlConfiguration StadConfig = YamlConfiguration.loadConfiguration((File)StadFile);
        StadConfig.set("StadData.Spawns.ballSpawn", (Object)"{}");
        StadConfig.set("StadData.Spawns.teamOneSpawn", (Object)"{}");
        StadConfig.set("StadData.Spawns.teamTwoSpawn", (Object)"{}");
        StadConfig.set("StadData.Data.ranked", (Object)ranked);
        StadConfig.set("StadData.Data.physicsType", (Object)"classic");
        StadConfig.set("StadData.Data.runTime", (Object)300);
        StadConfig.set("StadData.Data.ready", (Object)false);
        try {
